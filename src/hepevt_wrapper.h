...
/* This definition of HEPEVT corresponds to FORTRAN definition:
     ...
     & JMOHEP(2,NMXHEP),JDAHEP(2,NMXHEP),PHEP(5,NMXHEP),VHEP(4,NMXHEP) INTEGER
   NEVHEP,NHEP,ISTHEP,IDHEP,JMOHEP,JDAHEP DOUBLE PRECISION PHEP,VHEP
*/

static const int NMXHEP =
    HEPMC3_HEPEVT_NMXHEP; //!< Number of particles in the HEPEVT structure
typedef HEPMC3_HEPEVT_PRECISION
    momentum_t; //!< Precision of the 4-momentum, time-space position and mass
 ...
 * Geneva, Switzerland, September 4-5, 1989, CERN 89-08 (Geneva, 1989), Vol. 3,
 * p. 327 Disk representation is given by Fortran WRITE/READ format.
 */
struct HEPEVT {
  ...
  int jmohep[NMXHEP]
            [2]; //!< Pointer to position of 1st and 2nd (or last!) mother
  int jdahep[NMXHEP]
            [2]; //!< Pointer to position of 1nd and 2nd (or last!) daughter
...
public:
  ...
  static bool GenEvent_to_HEPEVT(const GenEvent *evt) {
    return GenEvent_to_HEPEVT_static<HEPEVT_Wrapper>(evt);
  };
  /** @brief Convert HEPEVT to GenEvent*/
  static bool HEPEVT_to_GenEvent(GenEvent *evt) {
    return HEPEVT_to_GenEvent_static<HEPEVT_Wrapper>(evt);
  };
#endif
...
public:
  ...
  static int number_children(
      const int &index); //!< Get number of children from the range of daughters
  static int number_children_exact(
      const int &index); //!< Get number of children by counting
  static void set_event_number(const int &evtno) {
    hepevtptr->nevhep = evtno;
  } //!< Set event number
  static void set_number_entries(const int &noentries) {
    hepevtptr->nhep = noentries;
  } //!< Set number of entries
  static void set_status(const int &index, const int &status) {
    hepevtptr->isthep[index - 1] = status;
  } //!< Set status code
  static void set_id(const int &index, const int &id) {
    hepevtptr->idhep[index - 1] = id;
  } //!< Set PDG particle id
  static void set_parents(const int &index, const int &firstparent,
                          const int &lastparent); //!< Set parents
  static void set_children(const int &index, const int &firstchild,
                           const int &lastchild); //!< Set children
  static void set_momentum(const int &index, const double &px, const double &py,
                           const double &pz,
                           const double &e);           //!< Set 4-momentum
  static void set_mass(const int &index, double mass); //!< Set mass
  static void set_position(const int &index, const double &x, const double &y,
                           const double &z,
                           const double &t); //!< Set position in time-space
};
...
inline void HEPEVT_Wrapper::print_hepevt(std::ostream &ostr) {
  ...
  ostr << "  Nr   Type   Parent(s)  Daughter(s)      Px       Py       Pz      "
          " E    Inv. M."
       << std::endl;
...
inline void HEPEVT_Wrapper::print_hepevt_particle(int index,
                                                  std::ostream &ostr) {
  char buf[255]; // Note: the format is fixed, so no reason for complicated
                 // treatment

  sprintf(buf, "%5i %6i", index, hepevtptr->idhep[index - 1]);
  ostr << buf;
  sprintf(buf, "%4i - %4i  ", hepevtptr->jmohep[index - 1][0],
          hepevtptr->jmohep[index - 1][1]);
  ostr << buf;
  sprintf(buf, "%4i - %4i ", hepevtptr->jdahep[index - 1][0],
          hepevtptr->jdahep[index - 1][1]);
  ostr << buf;
  sprintf(buf, "%8.2f %8.2f %8.2f %8.2f %8.2f", hepevtptr->phep[index - 1][0],
          hepevtptr->phep[index - 1][1], hepevtptr->phep[index - 1][2],
          hepevtptr->phep[index - 1][3], hepevtptr->phep[index - 1][4]);
  ostr << buf << std::endl;
}
...
inline int HEPEVT_Wrapper::number_children_exact(const int &index) {
  int nc = 0;
  for (int i = 1; i <= hepevtptr->nhep; ++i)
    if (((hepevtptr->jmohep[i - 1][0] <= index &&
          hepevtptr->jmohep[i - 1][1] >= index)) ||
        (hepevtptr->jmohep[i - 1][0] == index) ||
        (hepevtptr->jmohep[i - 1][1] == index))
      nc++;
  return nc;
}

inline void HEPEVT_Wrapper::set_parents(const int &index,
...
inline void HEPEVT_Wrapper::set_children(const int &index,
...
inline void HEPEVT_Wrapper::set_momentum(const int &index, const double &px,
...
inline void HEPEVT_Wrapper::set_position(const int &index, const double &x,
...
inline bool HEPEVT_Wrapper::fix_daughters() {
  /*AV The function should be called  for a record that has correct particle
  ordering and mother ids. As a result it produces a record with ranges where
  the daughters can be found. Not every particle in the range will be a
  daughter. It is true only for proper events. The return tells if the record
  was fixed succesfully.
  */
  for (int i = 1; i <= HEPEVT_Wrapper::number_entries(); i++)
    for (int k = 1; k <= HEPEVT_Wrapper::number_entries(); k++)
      if (i != k)
        if ((HEPEVT_Wrapper::first_parent(k) <= i) &&
            (i <= HEPEVT_Wrapper::last_parent(k)))
          HEPEVT_Wrapper::set_children(
              i,
              (HEPEVT_Wrapper::first_child(i) == 0
                   ? k
                   : std::min(HEPEVT_Wrapper::first_child(i), k)),
              (HEPEVT_Wrapper::last_child(i) == 0
                   ? k
                   : std::max(HEPEVT_Wrapper::last_child(i), k)));
  bool is_fixed = true;
  for (int i = 1; i <= HEPEVT_Wrapper::number_entries(); i++)
    is_fixed = (is_fixed && (HEPEVT_Wrapper::number_children_exact(i) ==
                             HEPEVT_Wrapper::number_children(i)));
  return is_fixed;
}
