#ifndef PYHEPMC_HEPEVT_WRAPPER_H
#define PYHEPMC_HEPEVT_WRAPPER_H
...
bool fill_genevent_from_hepevt(
    HepMC3::GenEvent &evt, int event_number,
  ...
  */
  for (int i = 0; i < nentries; ++i) {
    ...
    p->set_momentum(
        FourVector(pa(i, 0) / momentum_scaling, pa(i, 1) / momentum_scaling,
                   pa(i, 2) / momentum_scaling, pa(i, 3) / momentum_scaling));
    p->set_status(psta(i));
    p->set_pid(pid(i));
  ...
  */
  ...
  for (int i = 0; i < nentries; ++i) {
    ...
    if (vi == vertex_map.end()) {
      ...
      v->set_position(
          FourVector(va(i, 0) / length_scaling, va(i, 1) / length_scaling,
                     va(i, 2) / length_scaling, va(i, 3) / length_scaling));
